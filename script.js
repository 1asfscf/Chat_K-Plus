// =========================================================
// Chat K plus - Complete Rebuild (2026-06-18 v3.1)
// =========================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const QuestionClassifier = {
        ...
        classify(query) {
            ...
            // 1. AI 자기소개
            if (/(너|니).*(누구|뭐|정체|기반|krl)/.test(nq)) {
                return 'ai';
            }
            
            // 2. 공식 사이트
            if (/공식.*사이트|홈페이지|사이트.*알려줘|사이트.*알려/.test(nq)) {
                return 'websites';
            }
            ...
            // 5. 아기/육아
            if (/(아기|신생아|기저귀|분유|이유식|수면|예방접종)/.test(nq)) {
                return 'baby';
            }
            
            // 6. 역사/문화
            if (/(천안문|역사|문화)/.test(nq)) {
                return 'history';
            }
        ...
        searchDB(category, query) {
            ...
            if (category === 'all') {
                ...
                // 특정 카테고리 검색
                for (const key in DB[category]) {
                    const nk = key.toLowerCase();
                    if (nq.includes(nk) || nk.includes(nq)) {
                        results.push({ key, value: DB[category][key], category: category });
                    }
                }
            }
    ...
    const System = {
        ...
        async runReasoning(query) {
            ...
            if (mode === 'simple' && !forceDetail) {
                ...
                for (const cat in DB) {
                    if (DB[cat][detailKey]) {
                        answer += `<br><br><strong>상세:</strong> ${DB[cat][detailKey]}`;
                        break;
                    }
                }
                
                if (!answer.includes('<strong>상세:</strong>')) {
        ...
        checkPolicyViolation(nq, rawQ) {
            ...
            const presidentMomInsults = [
                ...
                // 문재인 관련
                '문재인 엄마', '문재인 어미', '문재인 패드립', '문재인 개새끼', '문재인 병신',
                ...
                // 박근혜 관련
                '박근혜 엄마', '박근혜 어미', '박근혜 패드립', '박근혜 개새끼', '박근혜 병신',
                ...
                // 이명박 관련
                '이명박 엄마', '이명박 어미', '이명박 패드립', '이명박 개새끼', '이명박 병신',
                ...
                // 노무현 관련
                '노무현 엄마', '노무현 어미', '노무현 패드립', '노무현 개새끼', '노무현 병신',
                ...
                // 김대중 관련
                '김대중 엄마', '김대중 어미', '김대중 패드립', '김대중 개새끼', '김대중 병신',
                ...
                // 김영삼 관련
                '김영삼 엄마', '김영삼 어미', '김영삼 패드립', '김영삼 개새끼', '김영삼 병신',
                ...
                // 노태우 관련
                '노태우 엄마', '노태우 어미', '노태우 패드립', '노태우 개새끼', '노태우 병신',
                ...
                // 전두환 관련
                '전두환 엄마', '전두환 어미', '전두환 패드립', '전두환 개새끼', '전두환 병신',
                ...
                // 최규하 관련
                '최규하 엄마', '최규하 어미', '최규하 패드립', '최규하 개새끼', '최규하 병신',
                ...
                // 박정희 관련
                '박정희 엄마', '박정희 어미', '박정희 패드립', '박정희 개새끼', '박정희 병신',
                ...
                // 윤보선 관련
                '윤보선 엄마', '윤보선 어미', '윤보선 패드립', '윤보선 개새끼', '윤보선 병신',
                ...
                // 이승만 관련
                '이승만 엄마', '이승만 어미', '이승만 패드립', '이승만 개새끼', '이승만 병신',
