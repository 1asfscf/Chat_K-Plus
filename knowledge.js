...
class ConversationContext {
    ...
    getContextualHint() {
        if (this.lastCategory && knowledgeBase[this.lastCategory]?.followUp) {
...
function searchKnowledgeBase(query, context = null) {
        ...
        if (!matchedPattern) {
            for (const keyword of data.keywords) {
                if (lowerQuery.includes(keyword.toLowerCase())) {
                    score += keyword.length * (data.priority || 1);
                }
            }
        }
