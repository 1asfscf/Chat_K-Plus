...
const knowledgeBase = {
  ...
  "종교": {
    ...
    sources: [],
    keywords: ['종교', '기독교', '불교', '이슬람', '힌두교', '유교', '하나님', '예수', '부처', '성경', '교회', '기도', '코란', '쿠란', '공자'],
    tags: ['종교', '문화'],
    needsReasoning: false
  },
  "정교회": {
  ...
  "윤회": {
    ...
    keywords: ['윤회', '삼사라', '환생', '업보', '열반', '해탈', '육도윤회'],
    tags: ['종교', '철학'],
    needsReasoning: false
  },
  "예수": {
...
function initApp() {
  ...
  // 예시 카드 동작 - 모바일 완전 대응
  document.querySelectorAll('.example-card,.prompt-card, [data-prompt], [data-example],.welcome-card').forEach(card => {
    bindTap(card, () => {
      const prompt = card.dataset.prompt || card.dataset.example || card.innerText.trim();
