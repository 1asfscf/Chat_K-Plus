...
const MODEL_IDENTITY = Object.freeze({
  ...
  engine: 'KRL',
  cutoff: '2025-09-04',
  desc: `나는 ${MODEL_NAME}이야. 스튜디오 페라리에서 제작한 AI야. Meta의 Muse Spark를 기반으로 하되, KRL(Knowledge Reasoning Layer) 엔진으로 한국 언어/문화에 최적화됐어. 역사 팩트체크, 개발, 건강 가이드, 영화, 음료, 일상 대화를 도와준다. 실시간 검색은 안 되고 2025-09-04까지 데이터로 학습했어.`
});
...
**MDN을 활용하는 팁**
- 검색창에 "mdn [찾을 내용]" 입력하면 바로 공식 문서
- 브라우저 호환성 표(BCD)로 크로스브라우징 확인
- 초보자는 '가이드' 섹션부터 시작 추천
...
목마를 땐 뭘 마실지 고민되면 나한테 물어봐! 다양한 음료 정보를 알려줄게.

**☕ 커피**
- **아메리카노**: 에스프레소 + 물. 칼로리 거의 없음
- **카페라떼**: 에스프레소 + 따뜻한 우유
- **카푸치노**: 에스프레소 + 우유 + 우유 거품
- **에스프레소**: 진한 커피 30ml. 카페인 강함
- **콜드브루**: 찬물로 12시간 추출. 부드럽고 쓴맛 적음
- 카페인 함량: 아메리카노 1잔 약 150mg

**🍵 차**
- **녹차**: 항산화 효과, 카테킨 풍부
- **홍차**: 완전 발효차. 스트레이트/밀크티로
- **보이차**: 후발효차. 숙성될수록 깊은 맛
- **허브차**: 캐모마일(숙면), 페퍼민트(소화)
...
더 자세한 음료 정보가 필요하면 말해줘 ${userName}! 🥤`,
    sources: [
      { title: "식품의약품안전처 식품영양성분DB", url: "https://www.foodsafetykorea.go.kr" }
    ],
    keywords: ['음료', '커피', '차', '녹차', '홍차', '콜라', '사이다', '주스', '물', '마실', '드링크', '라떼', '아메리카노', '에스프레소'],
    tags: ['일상', '건강', '식품'],
    needsReasoning: false,
    hasTable: true
  },
...
| 영화 | 감독 | 연도 | 주요 수상 |
|------|------|------|-----------|
| 기생충 | 봉준호 | 2019 | 아카데미 작품상 |
| 올드보이 | 박찬욱 | 2003 | 칸 심사위원대상 |
| 부산행 | 연상호 | 2016 | 칸 미드나잇스크리닝 |
| 헤어질 결심 | 박찬욱 | 2022 | 칸 감독상 |
...
const replies = {
  ...
  thanks: [
    `ㅇㅋ ${userName}. 더 궁금한 거 있어?`,
    `별거 아냐 ${userName}. 또 필요하면 불러줘!`,
    `ㄱㅅ ${userName}. 언제든 물어봐.`
  ],
  nameSet: [
    `알았어 ${userName}! 이제 그렇게 부를게.`,
    `ㅇㅋ ${userName}로 기억했어. 뭐부터 할까?`,
    `좋아 ${userName}. 편하게 말해.`
  ],
  reasoning: [
    `${userName}, 데이터 깊게 파는 중이야. 잠깐만 기다려줘.`,
    `1차 탐색 실패. 2차 추론 들어간다 ${userName}.`,
    `좀 더 찾아볼게 ${userName}. 3차 추론 중!`,
    `거의 다 왔어 ${userName}. 4차 추론 시도 중...`,
    `마지막이야 ${userName}. 5차 추론으로 최선을 다할게!`
  ],
  failed: [
    `${userName}, 5차까지 추론했는데 데이터가 없어. 더 정확한 정보가 필요하면 ${TEAM_EMAIL}로 피드백 보내줘. 네 의견이 나를 더 똑똑하게 만들어! 💪`,
    `미안 ${userName}. 이건 내 지식베이스에 없어. 하지만 포기하지 마. 다른 질문이라면 자신 있어!`
  ],
  blocked: [
    `${userName}, 그 질문은 Chat K Plus 정책상 답변이 어려워. 대신 과학, 기술, 역사, 건강, 영화, 종교, 음료 같은 주제는 자신 있게 답할 수 있어! 다른 궁금한 건 없을까? 😊`,
    `${userName}, 부적절한 내용이 감지됐어. Chat K Plus는 모두가 안전하게 사용할 수 있는 AI를 지향해. 다른 유익한 주제로 대화를 이어가자! 🌱`,
    `미안 ${userName}, 그 주제는 지원하지 않아. 하지만 포기하지 마! 내가 잘 아는 분야는 과학, 기술, 역사, 종교, 영화, 건강, 음료 등이 있어. 다른 질문 해볼래? ✨`
  ],
  stopped: [
    `⏸️ 답변이 중단되었어 ${userName}. 다른 질문이 있으면 말해줘. 언제든 기다리고 있을게!`,
    `${userName}, 답변 생성이 중단됐어. 다시 시도하려면 말해줘. 난 항상 여기 있어.`
  ]
};
