...
function sendMessage(predefinedText) {
    ...
    if (text.includes('이 질문 가르치기')) { openKnowledgeAdd(pendingKnowledgeInput || text); return; }
...
function generateFallbackResponse(u) { const kb = currentModel==='v1'?window.knowledgeBaseV1:currentModel==='v2'?window.knowledgeBaseV2:window.knowledgeBaseV3; const taught = kb.some(i => i.addedByUser && i.keywords && i.keywords.some(k => u.includes(k))); if (taught) return { reply: `"${u}" 아직 부족해요.`, table: null, chart: null, suggest: ['뭐 할 수 있어?', '과학'] }; return { reply: `"${u}" 데이터가 없어요. 😅\n\n💡 가르쳐주시겠어요?`, table: null, chart: null, suggest: ['📝 이 질문 가르치기', '뭐 할 수 있어?', '과학'] }; }
...
document.addEventListener('click', (e) => { if (e.target === document.getElementById('settingsModal')) closeSettings(); if (e.target === document.getElementById('knowledgeAddModal')) closeKnowledgeAdd(); if (e.target === document.getElementById('feedbackModal')) closeFeedbackModal(); });
document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { closeSettings(); closeKnowledgeAdd(); closeFeedbackModal(); } });
...
// ===== 지식 추가 =====
function openKnowledgeAdd(userInput) { if (currentModel !== 'v3') { addMessage('bot', '⚠️ Ultra 모델에서만 가능해요.', null, null, ['Ultra 모델로 전환']); return; } pendingKnowledgeInput = userInput; const kwEl = document.getElementById('knowledgeKeywords'); if (kwEl) kwEl.value = extractNewKeywords(userInput).join(', '); const catEl = document.getElementById('knowledgeCategory'); if (catEl) catEl.value = autoCategorizeFromText(userInput); document.getElementById('knowledgeAddModal').classList.add('active'); }
function closeKnowledgeAdd() { document.getElementById('knowledgeAddModal').classList.remove('active'); pendingKnowledgeInput = ''; }
function extractNewKeywords(text) { const sw = ['이', '가', '은', '는', '을', '를', '에', '의', '로', '뭐', '뭐야', '뭐냐', '알려줘', '설명해줘', '무엇', '어떤', '어떻게', '왜', '누가', '언제', '어디서', '무엇을', '그리고', '하지만', '그래서', '그럼', '또는']; return [...new Set(text.split(/[\s?？,，!.]+/).filter(w => w.length >= 2 && !sw.includes(w)))].slice(0, 6); }
function autoCategorizeFromText(text) { const cats = { '과학': ['과학', '물리', '화학', '생물', '중력', '우주', 'DNA'], '기술': ['컴퓨터', '코딩', '프로그래밍', '인터넷', 'AI', '파이썬'], '건강': ['운동', '수면', '다이어트', '영양', '건강'], '공부': ['공부', '학습', '영어', '수학', '시험'], '감정': ['힘들어', '슬퍼', '외로워', '불안', '위로'], '애니메이션': ['또봇', '도라에몽', '포켓몬', '원피스', '애니'], '게임': ['게임', '마리오', '소닉', '롤', '마인크래프트'] }; let b = '', bs = 0; for (let [c, k] of Object.entries(cats)) { let s = k.filter(kw => text.includes(kw)).length; if (s > bs) { bs = s; b = c; } } return bs > 0 ? b : ''; }
function submitKnowledge() { const kStr = document.getElementById('knowledgeKeywords').value.trim(); const reply = document.getElementById('knowledgeReply').value.trim(); const cat = document.getElementById('knowledgeCategory').value; if (!kStr || !reply) { alert('키워드와 답변을 모두 입력해주세요.'); return; } const keywords = kStr.split(/[,，]+/).map(k => k.trim()).filter(k => k.length > 0); if (keywords.length === 0) { alert('최소 1개 이상의 키워드가 필요해요.'); return; } const kb = currentModel === 'v1' ? window.knowledgeBaseV1 : currentModel === 'v2' ? window.knowledgeBaseV2 : window.knowledgeBaseV3; let found = false; for (let item of kb) { if (item.keywords && item.keywords.some(kw => keywords.includes(kw))) { item.reply = reply; item.keywords = [...new Set([...item.keywords, ...keywords])]; item.addedByUser = true; found = true; break; } } if (!found) { kb.push({ keywords, reply, addedByUser: true, addedAt: new Date().toISOString(), category: cat || '기타' }); } saveCustomKnowledge(kb); closeKnowledgeAdd(); removeLastTeachSuggestion(); setTimeout(() => { addMessage('bot', reply, null, null, ['더 자세히', '그래프로']); saveConversation(); updateInputState(); }, 300); showKnowledgeAddedToast(); }
function removeLastTeachSuggestion() { const msgs = document.querySelectorAll('.message.bot'); if (msgs.length === 0) return; const last = msgs[msgs.length - 1]; const sd = last.querySelector('.suggestions'); if (!sd) return; const btns = sd.querySelectorAll('.suggest-tag'); btns.forEach(b => { if (b.textContent.includes('가르치기')) { b.style.opacity = '0'; b.style.transition = 'opacity 0.3s'; setTimeout(() => b.remove(), 300); } }); setTimeout(() => { if (sd.querySelectorAll('.suggest-tag:not([style*="opacity: 0"])').length === 0) { sd.style.opacity = '0'; sd.style.transition = 'opacity 0.3s'; setTimeout(() => sd.remove(), 300); } }, 350); }
function saveCustomKnowledge(kb) { try { localStorage.setItem('chatKPlus_custom_knowledge_' + currentModel, JSON.stringify(kb.filter(i => i.addedByUser))); } catch (e) { } }
function loadCustomKnowledge() { ['v1', 'v2', 'v3'].forEach(m => { try { const s = localStorage.getItem('chatKPlus_custom_knowledge_' + m); if (s) { const items = JSON.parse(s); const kb = m === 'v1' ? window.knowledgeBaseV1 : m === 'v2' ? window.knowledgeBaseV2 : window.knowledgeBaseV3; items.forEach(i => { if (!kb.some(k => k.keywords && i.keywords && k.keywords.some(kw => i.keywords.includes(kw)))) kb.push(i); }); } } catch (e) { } }); }
function showKnowledgeAddedToast() { const t = document.createElement('div'); t.className = 'knowledge-toast'; t.innerHTML = '✅ 지식이 추가되었어요!'; document.body.appendChild(t); setTimeout(() => { t.style.opacity = '0'; t.style.transform = 'translateY(20px)'; setTimeout(() => t.remove(), 300); }, 2500); }
